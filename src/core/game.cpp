...
Game::Game(TaxConfig tax, SpecialConfig special, MiscConfig misc,
    ...
    : tax_(tax), special_(special), misc_(misc), rrCfg_(rrCfg),
      utilCfg_(utilCfg) {}
...
void Game::setBoard(unique_ptr<Board> board) { board_ = move(board); }
...
void Game::initCardDecks() {
  chanceDeck_.addCard(
      make_unique<ChanceCard>(ChanceEffect::GO_NEAREST_STATION));
  ...
  mt19937 rng(random_device{}());
    for (int i = 0; i < 4; ++i) skillDeck_.addCard(make_unique<MoveCard>());
    for (int i = 0; i < 3; ++i) skillDeck_.addCard(make_unique<DiscountCard>());
  for (int i = 0; i < 2; ++i)
    skillDeck_.addCard(make_unique<ShieldCard>());
  for (int i = 0; i < 2; ++i)
    skillDeck_.addCard(make_unique<TeleportCard>());
  for (int i = 0; i < 2; ++i)
    skillDeck_.addCard(make_unique<LassoCard>());
  for (int i = 0; i < 2; ++i)
    skillDeck_.addCard(make_unique<DemolitionCard>());
  skillDeck_.shuffle();
}

Player &Game::currentPlayer() { return *players_[turnOrder_[turnOrderIdx_]]; }
...
vector<Player *> Game::winners() const {
  ...
  sort(active.begin(), active.end(),
       [](Player *a, Player *b) { return *b < *a; });
  int topWorth = active[0]->netWorth();
  vector<Player *> tied;
  for (auto *p : active)
    if (p->netWorth() == topWorth)
      tied.push_back(p);
  if (tied.size() == 1)
    return tied;
  int maxProps = 0;
  for (auto *p : tied)
    maxProps = max(maxProps, (int)p->properties().size());
  vector<Player *> tied2;
  for (auto *p : tied)
    if ((int)p->properties().size() == maxProps)
      tied2.push_back(p);
  if (tied2.size() == 1)
    return tied2;
  int maxCards = 0;
  for (auto *p : tied2)
    maxCards = max(maxCards, p->handSize());
  vector<Player *> tied3;
  for (auto *p : tied2)
...
void Game::advanceTurn() {
  ...
  do {
    ...
    if (attempts > n)
      break;
  } while (!players_[turnOrder_[turnOrderIdx_]]->isActive() &&
           !players_[turnOrder_[turnOrderIdx_]]->isJailed());
  if (turnOrderIdx_ == 0)
    ++currentTurn_;
}
...
void Game::distributeSkillCard(Player& player) {
    ...
    card->onDraw(); // Kartu mengacak efeknya (jika ada) saat masuk ke tangan
    if (player.handFull()) {
...
void Game::movePlayer(Player &player, int steps, bool collectGoSalary) {
  ...
  if (collectGoSalary && newPos < oldPos)
    awardGoSalary(player);
  player.setPosition(newPos);
  logger_.log(currentTurn_, player.username(), "GERAK",
              "Maju " + to_string(steps) + " petak -> " +
                  board_->getTile(newPos)->name());
}

void Game::teleportPlayer(Player &player, int targetIndex) {
  player.setPosition(targetIndex);
  logger_.log(currentTurn_, player.username(), "TELEPORT",
              "Pindah ke " + board_->getTile(targetIndex)->name());
}

void Game::sendToJail(Player &player) {
  int jailIdx = 10;
  for (int i = 0; i < board_->size(); ++i) {
    if (board_->getTile(i)->type() == TileType::JAIL) {
      jailIdx = i;
      break;
    }
  }
...
void Game::awardGoSalary(Player &player) {
  ...
  logger_.log(currentTurn_, player.username(), "GAJI",
              "Melewati/berhenti di GO, menerima M" +
                  to_string(special_.goSalary));
}

void Game::cmdRollDice() {
  Player &player = currentPlayer();
  if (player.hasRolled() && !dice_.isDouble())
    throw logic_error("Kamu sudah melempar dadu pada giliran ini.");
  if (player.isJailed()) {
    handleJailTurn(player);
    return;
  }
  auto [d1, d2] = dice_.roll();
  logger_.log(currentTurn_, player.username(), "DADU",
              "Lempar: " + to_string(d1) + "+" + to_string(d2) + "=" +
                  to_string(d1 + d2));
...
void Game::cmdSetDice(int d1, int d2) {
  ...
  dice_.setRoll(d1, d2);
  logger_.log(currentTurn_, player.username(), "DADU",
              "Atur manual: " + to_string(d1) + "+" + to_string(d2) + "=" +
                  to_string(d1 + d2));
  if (dice_.doubleCount() == 3) {
    sendToJail(player);
    return;
...
void Game::handleJailTurn(Player &player) {
  if (player.jailTurns() >= 3) {
    if (!player.canAfford(special_.jailFine)) {
    ...
    player.resetJailTurns();
    logger_.log(currentTurn_, player.username(), "KELUAR_PENJARA",
                "Bayar denda M" + to_string(special_.jailFine) + " (wajib)");
    cmdRollDice();
    return;
  }
  auto [d1, d2] = dice_.roll();
  logger_.log(currentTurn_, player.username(), "DADU_PENJARA",
              "Lempar: " + to_string(d1) + "+" + to_string(d2));
  if (dice_.isDouble()) {
    player.setStatus(PlayerStatus::ACTIVE);
    player.resetJailTurns();
    ...
    logger_.log(currentTurn_, player.username(), "PENJARA",
                "Gagal keluar (percobaan " + to_string(player.jailTurns()) +
                    "/3)");
  }
}
...
void Game::processLanding(Player &player, int tileIndex, int diceTotal) {
  ...
  if (player.isShielded()) {
    logger_.log(currentTurn_, player.username(), "SHIELD",
                "ShieldCard melindungi di " + tile->name());
...
void Game::handlePropertyPurchase(Player &player, Property &prop) {
  if (prop.type() == PropertyType::RAILROAD ||
      prop.type() == PropertyType::UTILITY) {
    prop.setOwner(&player);
    prop.setStatus(PropertyStatus::OWNED);
    player.addProperty(&prop);
    refreshPropertyCounts(&player);
    logger_.log(currentTurn_, player.username(),
                prop.type() == PropertyType::RAILROAD ? "RAILROAD" : "UTILITY",
                prop.name() + " kini milik " + player.username() +
                    " (otomatis)");
    return;
  }

  bool wantsBuy = player.canAfford(prop.buyPrice()) && cb_.onOfferPurchase &&
                  cb_.onOfferPurchase(prop);
  if (!wantsBuy) {
    handleAuction(prop);
    return;
  }
  ...
  if (player.hasDiscount()) {
    ...
    logger_.log(currentTurn_, player.username(), "DISKON",
                prop.name() + " diskon -> M" + to_string(price));
  }
  ...
  player.addProperty(&prop);
  refreshPropertyCounts(&player);
  logger_.log(currentTurn_, player.username(), "BELI",
              "Beli " + prop.name() + " seharga M" + to_string(price));
}
...
void Game::handleRentPayment(Player &payer, Property &prop, int diceTotal) {
  ...
  if (rent <= 0)
    return;
  logger_.log(currentTurn_, payer.username(), "SEWA",
              "Bayar M" + to_string(rent) + " ke " + prop.owner()->username() +
                  " (" + prop.name() + ")");
  if (!payer.canAfford(rent)) {
    handleBankruptcy(payer, prop.owner());
    return;
  }
...
void Game::handleTaxPPH(Player &player) {
  ...
  logger_.log(currentTurn_, player.username(), "PAJAK",
              "PPH flat M" + to_string(flat));
}
...
void Game::handleTaxPBM(Player &player) {
  ...
  logger_.log(currentTurn_, player.username(), "PAJAK",
              "PBM flat M" + to_string(tax));
}

void Game::handleFestival(Player &player) {
  logger_.log(currentTurn_, player.username(), "FESTIVAL",
              "Mendarat di petak Festival");
  if (cb_.onFestival)
    cb_.onFestival(player);
}
...
void Game::applyFestival(Player &player, const string &code) {
  ...
  if (!prop || prop->type() != PropertyType::STREET)
    throw invalid_argument("Kode properti tidak valid.");
  if (prop->owner() != &player)
    throw invalid_argument("Properti bukan milikmu.");
  auto *street = static_cast<Street *>(prop);
  street->festival().boost();
  logger_.log(currentTurn_, player.username(), "FESTIVAL",
              prop->name() + ": sewa x" +
                  to_string(street->festival().multiplier()) +
                  " selama 3 giliran");
}

void Game::handleAuction(Property &prop) {
  logger_.log(currentTurn_, "SISTEM", "LELANG",
              "Lelang dimulai: " + prop.name());
}

void Game::finishAuction(Player &winner, Property &prop, int finalBid) {
  if (finalBid > 0)
    bank_.collect(winner, finalBid);
  prop.setOwner(&winner);
  prop.setStatus(PropertyStatus::OWNED);
  winner.addProperty(&prop);
  refreshPropertyCounts(&winner);
  logger_.log(currentTurn_, winner.username(), "LELANG",
              "Menang lelang " + prop.name() + " seharga M" +
                  to_string(finalBid));
}

void Game::handleBankruptcy(Player &debtor, Player *creditor) {
  logger_.log(currentTurn_, debtor.username(), "BANGKRUT",
              creditor ? "Kreditor: " + creditor->username()
                       : "Kreditor: Bank");
  ...
  if (creditor) {
    ...
    logger_.log(currentTurn_, debtor.username(), "BANGKRUT",
                "Semua aset dialihkan ke " + creditor->username());
  } else {
    for (auto *prop : debtor.properties()) {
    ...
    logger_.log(currentTurn_, debtor.username(), "BANGKRUT",
                "Semua properti dikembalikan ke Bank dan dilelang");
  }
...
void Game::cmdBuild(const string &code) {
  ...
  Property *prop = board_->getProperty(code);
  if (!prop || prop->type() != PropertyType::STREET)
    throw invalid_argument("Kode properti tidak valid atau bukan Street.");
  if (prop->owner() != &player)
    throw invalid_argument("Properti bukan milikmu.");
  auto *street = static_cast<Street *>(prop);
  if (!street->hasMonopoly())
    throw logic_error("Kamu belum memonopoli color group ini.");
  if (street->hasHotel())
    throw logic_error("Properti sudah memiliki hotel (maksimal).");
  auto groupStreets = board_->colorGroupStreets(street->colorGroup());
  int minLevel = INT_MAX;
  for (auto *s : groupStreets)
    minLevel = min(minLevel, s->buildingLevel());
  if (street->buildingLevel() > minLevel)
    throw logic_error("Pemerataan bangunan tidak terpenuhi.");
  int cost = street->nextBuildCost();
  if (!player.canAfford(cost))
    throw runtime_error("Uang tidak cukup untuk membangun.");
  bank_.collect(player, cost);
  street->addBuilding();
  string label = street->hasHotel()
                     ? "Hotel"
                     : to_string(street->buildingLevel()) + " rumah";
  logger_.log(currentTurn_, player.username(), "BANGUN",
              prop->name() + " -> " + label + " (biaya M" + to_string(cost) +
                  ")");
}
...
void Game::cmdMortgage(const string &code) {
  ...
  if (!prop || prop->owner() != &player)
    throw invalid_argument("Properti tidak ditemukan atau bukan milikmu.");
  if (prop->isMortgaged())
    throw logic_error("Properti sudah digadaikan.");
  if (prop->type() == PropertyType::STREET) {
    auto *street = static_cast<Street *>(prop);
    ...
    if (hasBldg) {
      for (auto *s : group) {
        ...
        if (refund > 0) {
          ...
          logger_.log(currentTurn_, player.username(), "JUAL_BANGUNAN",
                      s->name() + " -> refund M" + to_string(refund));
        }
      }
    }
  }
  ...
  logger_.log(currentTurn_, player.username(), "GADAI",
              prop->name() + " digadaikan, menerima M" +
                  to_string(prop->mortgageValue()));
}
...
void Game::cmdRedeem(const string &code) {
  ...
  if (!prop || prop->owner() != &player)
    throw invalid_argument("Properti tidak ditemukan atau bukan milikmu.");
  if (!prop->isMortgaged())
    throw logic_error("Properti tidak sedang digadaikan.");
  int cost = prop->buyPrice();
  if (!player.canAfford(cost))
    throw runtime_error("Uang tidak cukup untuk menebus. Harga tebus: M" +
                        to_string(cost));
  bank_.collect(player, cost);
  prop->setStatus(PropertyStatus::OWNED);
  logger_.log(currentTurn_, player.username(), "TEBUS",
              prop->name() + " ditebus, bayar M" + to_string(cost));
}
...
void Game::cmdUseSkillCard(int handIndex) {
  ...
  logger_.log(currentTurn_, player.username(), "KARTU",
              "Pakai " + card->description());
}
...
void Game::cmdDropCard(int handIndex) {
  ...
  player.removeFromHand(handIndex);
  skillDeck_.discard(card);
  logger_.log(currentTurn_, player.username(), "DROP_KARTU",
              "Membuang " + card->description());
}
